...
type Navigator = (pageId: string) => void;
let navigateToPage: Navigator | null = null;
...
function initRipples(): void {
  document.addEventListener('click', (e) => {
    const btn = closestElement(e.target)?.closest<HTMLElement>('.btn');
    if (!btn) return;
...
export function launchConfetti(duration = 2500): void {
...
function initTooltips(): void {
  ...
  document.addEventListener(
    'mouseenter',
    (e) => {
      const el = closestElement(e.target)?.closest<HTMLElement>('[data-tooltip]');
      if (!el) return;
      if (tip) tip.remove();
  ...
  document.addEventListener(
    'mouseleave',
    (e) => {
      if (closestElement(e.target)?.closest('[data-tooltip]') && tip) {
        tip.remove();
        tip = null;
      }
    },
...
export function setWater(amount: number): void {
...
export function initChartHover(): void {
  document.addEventListener(
    'mouseenter',
    (e) => {
      const bar = closestElement(e.target)?.closest<HTMLElement>('.chart-bar');
      if (!bar) return;
...
export function initLongPress(): void {
  document.addEventListener('pointerdown', (e) => {
    const item = closestElement(e.target)?.closest<HTMLElement>('.exercise-item');
    if (!item) return;
...
function initKeyboardShortcuts(): void {
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === '?' && !e.ctrlKey) {
      Toast.show(
        '⌨️ Shortcuts: 1-Dashboard 2-Planner 3-Progress 4-AI Coach 5-Profile',
        'info',
        4000
      );
    }
