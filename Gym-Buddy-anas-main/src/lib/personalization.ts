...
export interface Accent {
  ...
  hex: string;
  hex2: string;
  /** Same hue darkened for legible accent TEXT on white (light mode). */
  deep: string;
  rgb: string;
}
...
export interface ApplyAccentOpts {
  persist?: boolean;
}
...
export function applyAccent(id: string, { persist = true }: ApplyAccentOpts = {}): Accent {
...
export function applyTheme(id: string, { persist = true }: { persist?: boolean } = {}): ThemeId {
  const theme: ThemeId = id === 'light' ? 'light' : 'dark';
...
/** Boot the saved theme without a visible cross-fade on first paint. */
